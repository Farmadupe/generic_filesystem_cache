...
use crate::processing_fs_cache::ProcessingFsCache;
...
impl FileSet {
        ...
        T: DeserializeOwned + Serialize + Send + Sync + Clone,
    {
        self.enumerated_paths = cache
            ...
            .into_par_iter()
            .filter(|k| {
                any_item_includes(&self.source_paths, k) && !any_item_includes(&self.excl_paths, k)
            })
            .collect()
    }

    fn should_keep(&self, x: &walkdir::DirEntry) -> bool {
        x.path().is_file() && !any_item_includes(&self.excl_paths, x.path())
    }
}
...
pub fn is_ancestor_of(reference: impl AsRef<Path>, cand: impl AsRef<Path>) -> bool {
    cand.as_ref()
        .ancestors()
        .any(|anc| reference.as_ref() == anc)
}

fn any_item_includes(
    references: impl IntoIterator<Item = impl AsRef<Path>>,
    cand: impl AsRef<Path>,
) -> bool {
